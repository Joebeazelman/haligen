...
def execute_subprocess(cmd_and_args: str, current_working_dir: Path):
    ...
    # Show finish message, it's also useful because bar cannot start new line on console, why?
...
def is_utility_in_path_var(utility):
    ...
    if result is None:
        try:
            ...
            logging.error(
                f'\t\t\tUtility "{utility}" not found in PATH. Error #{e.errno}.')
    return Path(result) if result is not None else None


# noinspection PyUnusedLocal
def file_contains(filename: Path, content: str) -> bool:
    try:
        with open(filename, 'r') as f:
            return content in f.read()
    except IOError as e:
        logging.error(f'Error reading file "{filename}". Error: {e}')
        exit(-1)


def insert_lines_into_file(file_path: Path, line_index: int, content: str):
    try:
    ...
    if lines[line_index] != content.splitlines()[0]:
        ...
        logging.warning(
            f'\t\tCannot insert lines into file "{os.path.basename(file_path)}" at line {line_index}. Lines are '
            f'already inserted.')
    try:
